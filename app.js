...
state.foodLookupCache=store.get('foodLookupCache',{});
state.pendingMealIngredients=[];
const BASE_SCHEDULE=[
...
state.photoEntries=store.get('photoEntries',[]);
state.skinfoldEntries=store.get('skinfoldEntries',[]);
state.checkInDate=store.get('checkInDate',localISO());
...
function dashboard(){
  const d=todaySchedule(),plan=assignedTrainingPlan(),start=state.weights[0]||state.profile.weight,current=state.profile.weight,target=state.profile.target;
  ...
  const hour=new Date().getHours(),greeting=hour<6?'Goedenacht':hour<12?'Goedemorgen':hour<18?'Goedemiddag':'Goedenavond';
  return `<section class="welcome-row"><div><p>${greeting}</p><h2>Klaar voor vandaag?</h2></div><button class="avatar" onclick="goToTab('profile')" aria-label="Open profiel">K</button></section>
  <section class="dashboard-grid">
    ...
    <button class="dash-card plan-card ${plan?'':'no-plan'}" onclick="goToTab('training')"><div class="card-label">Huidig trainingsplan</div>${plan?`<div class="plan-status"><span>Van je coach</span><b>${esc(plan.title)}</b></div><p>${esc(plan.description)}</p><div class="card-link">Bekijk mijn plan <span>→</span></div>`:`<div class="plan-status"><span>Nog niet toegewezen</span><b>Maak je eigen training</b></div><p>Je coach heeft nog geen schema gekoppeld. Gebruik de workout builder om zelf te starten.</p><div class="card-link">Open workout builder <span>→</span></div>`}</button>
...
function setCheckInDate(value){state.checkInDate=value||localISO();store.set('checkInDate',state.checkInDate);render()}
function checkInDateControl(){return `<label class="checkin-date-control"><span>Meetdatum</span><input type="date" value="${state.checkInDate}" max="${localISO()}" onchange="setCheckInDate(this.value)"></label>`}
function resizeProgressPhoto(file){return new Promise((resolve,reject)=>{const reader=new FileReader();reader.onerror=reject;reader.onload=()=>{const img=new Image();img.onerror=reject;img.onload=()=>{const max=1000,scale=Math.min(1,max/Math.max(img.width,img.height)),canvas=document.createElement('canvas');canvas.width=Math.round(img.width*scale);canvas.height=Math.round(img.height*scale);canvas.getContext('2d').drawImage(img,0,0,canvas.width,canvas.height);resolve(canvas.toDataURL('image/jpeg',.72))};img.src=reader.result};reader.readAsDataURL(file)})}
async function saveProgressPhoto(input,view){const file=input.files?.[0];if(!file)return;try{const data=await resizeProgressPhoto(file),existing=state.photoEntries.find(x=>x.date===state.checkInDate&&x.view===view);if(existing)data&&Object.assign(existing,{data,updatedAt:Date.now()});else state.photoEntries.push({id:Date.now(),date:state.checkInDate,view,data,updatedAt:Date.now()});store.set('photoEntries',state.photoEntries);render();toast(`Foto opgeslagen bij ${new Date(state.checkInDate+'T12:00:00').toLocaleDateString('nl-NL')}`)}catch(error){console.error(error);toast('Foto kon niet worden opgeslagen. Kies een kleinere foto.')}}
function deleteProgressPhoto(id){state.photoEntries=state.photoEntries.filter(x=>x.id!==id);store.set('photoEntries',state.photoEntries);render();toast('Foto verwijderd')}
function saveSkinfoldEntry(){const values=Object.fromEntries([...document.querySelectorAll('[data-skinfold]')].map(input=>[input.dataset.skinfold,Math.max(0,+input.value||0)])),filled=Object.values(values).filter(Boolean);if(!filled.length)return toast('Vul minimaal één huidplooi in');const existing=state.skinfoldEntries.find(x=>x.date===state.checkInDate),entry={id:existing?.id||Date.now(),date:state.checkInDate,values,total:Object.values(values).reduce((a,b)=>a+b,0)};if(existing)Object.assign(existing,entry);else state.skinfoldEntries.push(entry);store.set('skinfoldEntries',state.skinfoldEntries);render();toast(existing?'Meting bijgewerkt':'Huidplooimeting opgeslagen')}
function deleteSkinfoldEntry(id){state.skinfoldEntries=state.skinfoldEntries.filter(x=>x.id!==id);store.set('skinfoldEntries',state.skinfoldEntries);render();toast('Meting verwijderd')}
function checkInV2(){
  const p=state.checkInPrefs,entries=[...state.weightEntries].sort((a,b)=>a.date.localeCompare(b.date)),values=entries.map(x=>x.weight),first=entries[0]?.weight||state.profile.weight,last=entries.at(-1)?.weight||state.profile.weight,delta=(last-first).toFixed(1),tabs=[p.weight&&['weight','Gewicht'],p.photos&&['photos',"Foto's"],p.skinfold&&['skinfold','Huidplooien']].filter(Boolean),dateLabel=date=>new Date(date+'T12:00:00').toLocaleDateString('nl-NL',{day:'numeric',month:'short',year:'numeric'});
  const top=`<div class="checkin-top"><div><p class="card-label">Jouw voortgang</p><h2>Check-in per datum</h2><p>Bewaar gewicht, foto's en huidplooien op de dag waarop je meet.</p></div><button class="settings-btn" onclick="document.querySelector('.checkin-settings').classList.toggle('open')">Instellen</button></div><div class="checkin-settings"><b>Zichtbare onderdelen</b><label><input type="checkbox" ${p.weight?'checked':''} onchange="toggleCheckIn('weight')"> Gewicht & grafiek</label><label><input type="checkbox" ${p.photos?'checked':''} onchange="toggleCheckIn('photos')"> Voortgangsfoto's</label><label><input type="checkbox" ${p.skinfold?'checked':''} onchange="toggleCheckIn('skinfold')"> Huidplooimeting</label></div><div class="checkin-tabs">${tabs.map(([id,label])=>`<button class="${state.checkInTab===id?'active':''}" onclick="setCheckInTab('${id}')">${label}</button>`).join('')}</div>${checkInDateControl()}`;
  if(state.checkInTab==='weight'&&p.weight){const current=state.weightEntries.find(x=>x.date===state.checkInDate)?.weight??last;return top+`<section class="measure-card"><div class="measure-head"><div><span>Gewichtsontwikkeling</span><strong>${last} kg</strong></div><span class="trend ${+delta>0?'neutral':''}">${+delta>0?'+':''}${delta} kg</span></div><svg class="weight-chart" viewBox="0 0 300 108" role="img" aria-label="Gewicht per datum"><line x1="8" y1="92" x2="292" y2="92"></line><polyline points="${graphPoints(values)}"></polyline>${graphPoints(values).split(' ').map((pt,i)=>{const[x,y]=pt.split(',');return `<circle cx="${x}" cy="${y}" r="4"><title>${entries[i]?dateLabel(entries[i].date)+': '+entries[i].weight+' kg':''}</title></circle>`}).join('')}</svg><div class="dated-weight-form"><label><span>Datum</span><input id="checkWeightDate" type="date" value="${state.checkInDate}" max="${localISO()}" onchange="setCheckInDate(this.value)"></label><label><span>Gewicht</span><div><input id="checkWeight" type="number" step="0.1" value="${current}"><b>kg</b></div></label><button onclick="saveCheckWeightFromSelectedDate()">Meting opslaan</button></div></section><section><div class="section-head"><div><h2>Vergelijk per datum</h2><p>${entries.length} meetmomenten</p></div></div><div class="list-card weight-history">${[...entries].reverse().map((x,i)=>`<div class="weight-history-row"><div><b>${dateLabel(x.date)}</b><span>${i===0?'Meest recent':'Eerdere meting'}</span></div><strong>${x.weight} kg</strong><button onclick="deleteWeightEntry(${x.id})" aria-label="Verwijder meting">×</button></div>`).join('')||'<div class="empty compact">Nog geen gewichten opgeslagen.</div>'}</div></section>`}
  if(state.checkInTab==='photos'&&p.photos){const todays=state.photoEntries.filter(x=>x.date===state.checkInDate),history=[...state.photoEntries].sort((a,b)=>b.date.localeCompare(a.date));return top+`<section class="measure-card"><div class="measure-head"><div><span>Voortgangsfoto's</span><strong>${dateLabel(state.checkInDate)}</strong></div></div><div class="photo-grid photo-grid-three">${[['front','Voorkant'],['side','Zijkant'],['back','Achterkant']].map(([id,label])=>{const photo=todays.find(x=>x.view===id);return `<label class="photo-slot ${photo?'has-photo':''}" style="${photo?`background-image:linear-gradient(rgba(0,0,0,.08),rgba(0,0,0,.55)),url('${photo.data}')`:''}"><input type="file" accept="image/*" capture="environment" onchange="saveProgressPhoto(this,'${id}')"><span>＋</span><b>${label}</b><small>${photo?'Tik om te vervangen':'Foto toevoegen'}</small></label>`}).join('')}</div></section><section><div class="section-head"><div><h2>Foto's vergelijken</h2><p>Alle foto's staan bij hun echte meetdatum.</p></div></div><div class="dated-photo-history">${history.map(x=>`<article><img src="${x.data}" alt="${esc(x.view)} op ${dateLabel(x.date)}"><div><b>${dateLabel(x.date)}</b><span>${x.view==='front'?'Voorkant':x.view==='side'?'Zijkant':'Achterkant'}</span></div><button onclick="deleteProgressPhoto(${x.id})" aria-label="Verwijder foto">×</button></article>`).join('')||'<div class="empty compact">Nog geen foto’s opgeslagen.</div>'}</div></section>`}
  const folds=['Borst','Buik','Dij','Triceps','Subscapulair','Suprailiacaal','Midaxillair'],selected=state.skinfoldEntries.find(x=>x.date===state.checkInDate),history=[...state.skinfoldEntries].sort((a,b)=>b.date.localeCompare(a.date));return top+`<section class="measure-card"><div class="measure-head"><div><span>Huidplooimeting</span><strong>7-punts · ${dateLabel(state.checkInDate)}</strong></div><span class="trend neutral">mm</span></div><div class="skinfold-grid">${folds.map(name=>`<label>${name}<input data-skinfold="${name}" type="number" inputmode="decimal" step="0.1" value="${selected?.values?.[name]||''}" placeholder="mm"></label>`).join('')}</div><button class="primary" onclick="saveSkinfoldEntry()">Meting opslaan</button></section><section><div class="section-head"><div><h2>Metingen vergelijken</h2><p>${history.length} meetmomenten</p></div></div><div class="list-card skinfold-history">${history.map(x=>`<article><div><b>${dateLabel(x.date)}</b><span>${Object.entries(x.values).filter(([,v])=>v).map(([k,v])=>`${k} ${v} mm`).join(' · ')}</span></div><strong>${Number(x.total.toFixed(1))} mm</strong><button onclick="deleteSkinfoldEntry(${x.id})">×</button></article>`).join('')||'<div class="empty compact">Nog geen huidplooimetingen opgeslagen.</div>'}</div></section>`
}
function saveCheckWeightFromSelectedDate(){const date=state.checkInDate,w=parseFloat($('#checkWeight')?.value);if(!date||!(w>35&&w<250))return toast('Vul een geldig gewicht in');const existing=state.weightEntries.find(x=>x.date===date);if(existing)existing.weight=w;else state.weightEntries.push({id:Date.now(),date,weight:w});syncWeightsFromEntries();render();toast(existing?'Meting bijgewerkt':'Nieuwe meting opgeslagen')}
checkIn=checkInV2;
...
function reportDateFromText(text){const value=String(text),numeric=value.match(/\b([0-3]?\d)[-\/.]([01]?\d)[-\/.](20\d{2})\b/),iso=value.match(/\b(20\d{2})[-\/.]([01]?\d)[-\/.]([0-3]?\d)\b/);if(numeric)return `${numeric[3]}-${String(numeric[2]).padStart(2,'0')}-${String(numeric[1]).padStart(2,'0')}`;if(iso)return `${iso[1]}-${String(iso[2]).padStart(2,'0')}-${String(iso[3]).padStart(2,'0')}`;const months={januari:1,februari:2,maart:3,april:4,mei:5,juni:6,juli:7,augustus:8,september:9,oktober:10,november:11,december:12},words=value.toLowerCase().match(/\b([0-3]?\d)\s+(januari|februari|maart|april|mei|juni|juli|augustus|september|oktober|november|december)\s+(20\d{2})\b/);return words?`${words[3]}-${String(months[words[2]]).padStart(2,'0')}-${String(words[1]).padStart(2,'0')}`:''}
...
async function analyzeLabUpload(input){
  ...
  try{const text=file.type==='application/pdf'||file.name.toLowerCase().endsWith('.pdf')?await extractPdfText(file):await extractImageText(file),results=extractLabValues(text),date=reportDateFromText(text);state.labDraft={fileName:file.name,date,results};render();setLabUploadStatus(`${results.length} waarden herkend.${date?' Meetdatum gevonden: '+new Date(date+'T12:00:00').toLocaleDateString('nl-NL'):' Controleer en kies de meetdatum uit het rapport.'}`,'success')}
  catch(error){console.error(error);state.labDraft={fileName:file.name,date:'',results:[]};render();setLabUploadStatus('Automatische herkenning lukte niet. Kies de meetdatum en voeg de waarden handmatig toe.','error')}
  finally{state.labUploadBusy=false}
}
function addLabDraftRow(){if(!state.labDraft)state.labDraft={fileName:'Handmatige invoer',date:'',results:[]};const used=new Set(state.labDraft.results.map(x=>x.name)),template=LAB_RESULTS.find(x=>!used.has(x.name))||LAB_RESULTS[0];state.labDraft.results.push({...template,value:''});render()}
...
function saveLabDraft(){
  if(!state.labDraft)return;const date=$('#labDraftDate')?.value,rows=[...document.querySelectorAll('[data-lab-draft-row]')],results=rows.map(row=>{const template=LAB_RESULTS.find(x=>x.name===row.querySelector('select').value),value=row.querySelector('input').value.trim();return value&&template?{...template,value,status:labStatusFor(template,value)}:null}).filter(Boolean);if(!date){toast('Kies eerst de meetdatum die op het rapport staat');return}if(!results.length){toast('Voeg minimaal één waarde toe');return}
...
function openNutritionMeal(meal){state.activeMeal=meal;store.set('activeMeal',meal);state.productResults=[];render();window.scrollTo({top:0,behavior:'smooth'})}
...
const FOOD_ALIASES={
  havermout:'havermout',oats:'havermout',whey:'whey','whey eiwit':'whey','whey proteine':'whey','whey protein':'whey','eiwitpoeder':'whey','proteinepoeder':'whey',
...
const PORTION_RULES=[
  {test:/\bei(eren)?\b/,grams:50,label:'middelgroot ei zonder schaal'},
  {test:/\b(banaan|bananen)\b/,grams:120,label:'middelgrote banaan zonder schil'},
  {test:/\b(appel|appels)\b/,grams:150,label:'middelgrote appel'},
  {test:/\b(volkoren)?brood|boterham/,unit:/snee|boterham/,grams:35,label:'snee brood'},
  {test:/\bwhey|eiwitpoeder|prote[iï]nepoeder/,unit:/scoop|schep/,grams:30,label:'scoop'},
  {test:/\bpindakaas/,unit:/eetlepel|el/,grams:15,label:'eetlepel'},
...
function portionFor(item,food){
  const unit=item.unit,metric={kg:1000,g:1,gr:1,gram:1,ml:1,cl:10,l:1000};if(metric[unit])return {grams:item.amount*metric[unit],label:`${item.amount} ${unit}`};
  const normalized=normalizeFoodTerm(item.term),rule=PORTION_RULES.find(x=>x.test.test(normalized)&&(!x.unit||x.unit.test(unit)));if(rule)return {grams:item.amount*rule.grams,label:`${item.amount} × ${rule.label}`};
  if(food?.servingGrams)return {grams:item.amount*food.servingGrams,label:`${item.amount} × portie van ${food.servingGrams} g`};return null;
}
function nutritionFromResolved(item,resolved){const portion=portionFor(item,resolved.food);if(!portion)return {...item,resolved,error:'Hoeveel gram is één stuk of portie? Voeg een gewicht toe.'};const factor=portion.grams/100,food=resolved.food;return {...item,resolved,food,grams:portion.grams,portionLabel:portion.label,kcal:food.kcal*factor,carbs:food.carbs*factor,protein:food.protein*factor,fat:food.fat*factor,source:resolved.source}}
function parseMealSentence(text){const matched=[],unmatched=[];for(const item of mealIngredientsFromText(text)){const food=localFoodForTerm(item.term);if(!food){unmatched.push(item.term);continue}const row=nutritionFromResolved(item,{food,source:'Karada productdatabase'});if(row.error)unmatched.push(item.term);else matched.push(row)}return {matched,unmatched,ingredients:mealIngredientsFromText(text)}}
async function resolveMealIngredient(item){const local=localFoodForTerm(item.term);if(local)return {food:local,source:'Karada productdatabase'};const key=normalizeFoodTerm(item.term),cached=state.foodLookupCache?.[key];if(cached)return cached;const url=`https://world.openfoodfacts.org/cgi/search.pl?search_terms=${encodeURIComponent(item.term)}&search_simple=1&action=process&json=1&page_size=16&fields=code,product_name,product_name_nl,brands,nutriments,serving_quantity,serving_size,product_quantity,quantity`;const res=await fetch(url,{signal:AbortSignal.timeout(9000)}),json=await res.json(),words=key.split(/\s+/).filter(x=>x.length>1),candidates=(json.products||[]).filter(p=>(p.product_name_nl||p.product_name)&&productMacros(p).kcal>0).map(p=>{const name=normalizeFoodTerm(`${p.product_name_nl||''} ${p.product_name||''} ${p.brands||''}`),hits=words.filter(w=>name.includes(w)).length,score=hits*3+(name.includes(key)?8:0)-(words.length-hits)*2;return {p,score,hits}}).filter(x=>x.hits>0).sort((a,b)=>b.score-a.score),best=candidates[0],product=best?.p;if(!product)return null;const m=productMacros(product),servingGrams=+(product.serving_quantity||String(product.serving_size||'').match(/[\d.,]+/)?.[0]?.replace(',','.')||0),result={food:{id:product.code||`off-${key}`,name:product.product_name_nl||product.product_name,kcal:m.kcal,carbs:m.carbs,protein:m.protein,fat:m.fat,servingGrams},source:`Open Food Facts${product.brands?' · '+product.brands:''}`,confidence:best.score};state.foodLookupCache={...(state.foodLookupCache||{}),[key]:result};store.set('foodLookupCache',state.foodLookupCache);return result}
...
function previewMealSentence(){const box=$('#mealSentencePreview'),input=$('#mealSentence');if(!box||!input)return;state.pendingMealIngredients=[];const parsed=parseMealSentence(input.value),count=parsed.ingredients?.length||0;box.innerHTML=count?`${mealSentenceSummary(parsed)}${parsed.unmatched.length?`<small>${parsed.unmatched.length} product${parsed.unmatched.length===1?'':'en'} of merk wordt online opgezocht voordat je iets toevoegt.</small>`:''}`:'<span>Bijv. 3 eieren, 2 sneetjes volkorenbrood en 30 g MyProtein Impact Whey.</span>';const button=$('#analyzeMealButton');if(button){button.textContent='Producten herkennen';button.onclick=analyzeMealSentence}}
function pendingMealHtml(rows){return rows.map(row=>row.error?`<article class="meal-match error"><div><b>${esc(row.term)}</b><span>${esc(row.error)}</span></div></article>`:`<article class="meal-match"><div><b>${esc(row.food.name)}</b><span>${esc(row.portionLabel)} · ${esc(row.source)}</span></div><strong>${Math.round(row.kcal)} kcal</strong><small>${row.carbs.toFixed(1)} g kh · ${row.protein.toFixed(1)} g eiwit · ${row.fat.toFixed(1)} g vet</small></article>`).join('')}
async function analyzeMealSentence(){const input=$('#mealSentence'),button=$('#analyzeMealButton'),box=$('#mealSentencePreview'),ingredients=mealIngredientsFromText(input?.value||'');if(!ingredients.length)return toast('Noem per product een hoeveelheid, bijvoorbeeld 3 eieren of 30 g whey');button.disabled=true;button.textContent='Producten en merken opzoeken…';const rows=[];for(const item of ingredients){try{const found=await resolveMealIngredient(item);rows.push(found?nutritionFromResolved(item,found):{...item,error:'Product of merk niet gevonden. Scan de barcode of zoek het product handmatig.'})}catch(error){console.warn('Product niet gevonden',item.term,error);rows.push({...item,error:'Productdatabase tijdelijk niet bereikbaar.'})}}state.pendingMealIngredients=rows;box.innerHTML=pendingMealHtml(rows);button.disabled=false;if(rows.every(x=>!x.error)){button.textContent=`Bevestigen en toevoegen aan ${MEAL_SLOTS.find(x=>x.id===(state.activeMeal||'breakfast'))?.label}`;button.onclick=confirmMealSentence}else{button.textContent='Pas de tekst aan en probeer opnieuw';button.onclick=analyzeMealSentence}}
function confirmMealSentence(){const rows=state.pendingMealIngredients||[];if(!rows.length||rows.some(x=>x.error))return toast('Controleer eerst alle producten');const meal=state.activeMeal||'breakfast';rows.forEach((x,i)=>state.foodLog.push({id:Date.now()+i,date:state.nutritionDate,meal,foodId:x.food.id,name:x.food.name,grams:Number(x.grams.toFixed(1)),kcal:x.kcal,carbs:x.carbs,protein:x.protein,fat:x.fat,source:x.source}));store.set('foodLog',state.foodLog);state.pendingMealIngredients=[];render();toast(`${rows.length} gecontroleerde producten toegevoegd aan ${MEAL_SLOTS.find(x=>x.id===meal)?.label}`)}
function nutritionHub(){
  ...
  const quickEntry=`<section class="meal-sentence-card"><div><span class="card-label">Snel toevoegen aan ${slot.label}</span><h2>Beschrijf wat je hebt gegeten</h2><p>Gebruik grammen, stuks of porties en noem eventueel het merk. Controleer het gevonden product voordat je het toevoegt.</p></div><textarea id="mealSentence" oninput="previewMealSentence()" placeholder="Bijv. 3 eieren, 2 sneetjes volkorenbrood en 30 g MyProtein Impact Whey"></textarea><div id="mealSentencePreview" class="meal-sentence-preview"><span>Vul je maaltijd hierboven in.</span></div><button id="analyzeMealButton" onclick="analyzeMealSentence()">Producten herkennen</button></section>`;
